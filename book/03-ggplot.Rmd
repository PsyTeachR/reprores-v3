...
```{r setup-ggplot, results = 'hide', warning = FALSE, message = FALSE, verbatim = "r setup, include = FALSE"}
...
While the functions above are nice for quick visualisations, it's hard to make pretty, publication-ready plots. The package `r pkg("ggplot2")` (loaded with `tidyverse`) is one of the most common packages for creating beautiful visualisations.

`ggplot2` creates plots using a "grammar of graphics" where you add `r glossary("geom", "geoms")` in layers. It can be complex to understand at first, but it's very powerful once you have a mental model of how it works. 

A grammar of graphics (the "gg" in "ggplot") is a standardised way to describe the components of a graphic. `r pkg("ggplot2")` uses a layered grammar of graphics, in which plots are built up in a series of layers. It may be helpful to think about any picture as having multiple elements that sit semi-transparently over each other. A good analogy is old Disney movies where artists would create a background and then add moveable elements on top of the background via transparencies.
...
```{r layers, echo = FALSE, fig.width = 12, fig.height = 5}
#| fig.cap="Evolution of a layered plot"

theme_set(theme_grey())

a <- ggplot() + labs(title = "Layer 1")
b <- ggplot(pets, aes(x = pet, y = score, colour = country, fill = country)) + 
  labs(title = "Layer 2")
c <- b + geom_violin(alpha = 0.5) + 
  labs(title = "Layer 3")
d <- c + scale_fill_manual(values = c("orange", "orchid")) +
...
```{r geom-crossbar, fig.cap="Four different styles of vertical interval"}
...
```{r fig-chunk-example1, eval = FALSE, verbatim="r fig-pet1, fig.width=10, fig.height=3, out.width=\"100%\", fig.align=\"center\""}
#| fig.cap: "10x3 inches at 100% width centre aligned."
ggplot(pets, aes(weight, score, color = pet)) +
  geom_smooth(formula = y~x, method = lm)
```

```{r fig-chunk-example1-out, fig.width=10, fig.height=3, out.width="100%", echo = FALSE}
#| fig.cap: "10x3 inches at 100% width centre aligned."
ggplot(pets, aes(weight, score, color = pet)) +
  geom_smooth(formula = y~x, method = lm)
```

```{r fig-chunk-example2, eval = FALSE, verbatim="r fig-pet2, fig.width=5, fig.height=3, out.width=\"50%\", fig.align=\"left\""}
#| fig.cap: "5x3 inches at 50% width aligned left."
ggplot(pets, aes(weight, score, color = pet)) +
  geom_smooth(formula = y~x, method = lm)
```

```{r fig-chunk-example2-out, fig.width=5, fig.height=3, out.width="50%", fig.align="left", echo = FALSE}
#| fig.cap: "5x3 inches at 50% width aligned left."
ggplot(pets, aes(weight, score, color = pet)) +
  geom_smooth(formula = y~x, method = lm)
```
...
```{r facet-wrap, fig.cap="Facetted plot, facetting by country."}
...
```{r facet-wrap2, fig.cap="Facetted plot, facetting by age", warning=FALSE}
...
```{r facet-grid, fig.cap="Facetted plot"}
