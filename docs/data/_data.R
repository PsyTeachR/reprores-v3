...
make_dataset <- function(dataname, title, desc, vardesc = list(), filetype = "csv", source = NULL, write = TRUE, ct = readr::cols()) {
  ...
  if (filetype == "csv") {
    ...
    data <- readxl::read_xlsx(datafile)
  }
...
make_dataset("eq_data", "Empathy Quotient",
...
vardesc <- list(
  description = list(
    ...
    variation = "The type of Alexa",
    feedback = "The text feedback",
    verified_reviews = "Whether the review was verified (1) or not (0)"
  )
)

make_dataset("amazon_alexa", "Amazon Alexa Reviews",
             "This dataset contains text reviews as well as the 1-5 rating from customers who bought an Alexa device on Amazon.", vardesc,
...
vardesc <- list(
  description = list(
    Day = "The date in d/m/YYYY format",
    RiseTime = "Sunrise time in decimal hour format",
    SetTime = "Sunset time in decimal hour format"
  )
)

make_dataset("sunfact2021", "Glasgow Sun",
             "Sunrise and sunset times for every day of the year in Glasgow in 2021.", vardesc,
             source = "https://www.schoolsobservatory.org/learn/astro/nightsky/sunrs_set")
...
vardesc <- list(
  description = list(
    ...
    population = "The population (contains commas as thousands separator)"
  )
)

make_dataset("scottish_population", "Scottish Population",
             "The population for each county in Scotland in 2021.", vardesc)
